...
public class PointController {
    ...
    public Result<PageInfo<AdminPoint>> list(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        for (Point point : list) {
            ...
            User user = userService.findById(point.getVid());
            String helper = user == null ? "":user.getUsername();
            points.add(new AdminPoint(username,helper,point));
        }
        PageInfo<AdminPoint> pageInfo = new PageInfo<>(points);
